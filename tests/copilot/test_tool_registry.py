...
from copilot.backend.tools.registry import (
    ToolContext, ToolDefinition, _Registry,
)
...
def test_global_registry_loaded():
    ...
    expected = {
        "project_status", "dataset_audit", "data_pipeline_run",
        "train_start", "train_status", "train_history", "train_get_run",
        "train_cancel", "eval_run", "gate_apply", "eval_get_metrics",
        "deploy_push", "deploy_promote", "deploy_render_k8s",
        "registry_list", "serve_health", "serve_chat",
        "monitor_request_log", "monitor_quality", "monitor_drift",
        "config_list", "config_read", "config_edit",
    }
...
async def test_dangerous_tool_blocked_when_disabled():
    ...
    local_reg = _Registry()
    local_reg.register(ToolDefinition(
...
async def test_handler_exception_is_captured_not_raised():
    ...
    async def boom(args, ctx):  # noqa: ARG001
        raise RuntimeError("kaboom")

    local_reg = _Registry()
    local_reg.register(ToolDefinition(
        name="boom", description="...", args_model=A, handler=boom,
    ))
