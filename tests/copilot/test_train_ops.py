...
def test_read_training_log_via_pointer(repo: Path) -> None:
    ...
    (d / ".log_path").write_text("artifacts/copilot/training-logs/sft_20260704.log",
                                 encoding="utf-8")
...
def test_preflight_passes_with_good_sft_data(repo: Path) -> None:
    _write_jsonl(repo / "data" / "processed" / "train.jsonl",
...
def test_preflight_dpo_rejects_non_preference_data(repo: Path) -> None:
    (repo / "configs" / "train_dpo.yaml").write_text(
        "model:\n  base_model: hf/test\ntraining:\n  epochs: 1\n", encoding="utf-8")
    _write_jsonl(repo / "data" / "processed" / "preference_train.jsonl",
                 [{"messages": [{"role": "user", "content": "hi"}]}])  # not chosen/rejected
...
def test_estimate_lora_fits(repo: Path) -> None:
    r = train_ops.estimate(
        repo, method="sft",
        overrides={"model.base_model": "meta-llama/Llama-3.1-8B",
...
def test_log_endpoint(repo: Path) -> None:
    ...
    (d / ".log_path").write_text("artifacts/copilot/training-logs/sft_x.log",
                                 encoding="utf-8")
...
def test_estimate_endpoint(repo: Path) -> None:
    with _client(repo) as client:
        r = client.post("/api/train/estimate", json={
            "method": "sft",
            "overrides": {"model.base_model": "meta-llama/Llama-3.1-8B"}})
        assert r.status_code == 200
        body = r.json()
...
def test_run_config_present(repo: Path) -> None:
    ...
    (d / "run_meta.json").write_text(json.dumps({
        "dataset_hash": "abc123", "smoke_test": False,
...
def test_run_config_endpoint(repo: Path) -> None:
    ...
    with _client(repo) as client:
        r = client.get("/api/train/run-config",
                       params={"adapter_dir": "artifacts/adapter"})
        assert r.status_code == 200
